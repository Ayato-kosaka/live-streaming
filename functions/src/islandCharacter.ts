 ...
 * ## 動画も、絵と同じ形で持つ
 *
 * 投げ銭のアラートで絵のかわりに流す短い動画は
 * **`videos.{役どころ}` = `{url, bytes, seconds, w, h, at}`** に入れる。
 * 絵（`images.{plain|scene}`）と同じで、**役どころで鍵を切ってある。**
 * いまの役どころは `alert` ひとつで、2本目は `VIDEO_ROLES` に1語足すだけ。
 *
 * **旧い `videoUrl`（文字列1本）も当分いっしょに返す。** OBS
 * （`app/alertbox/`）はまだそちらしか見ていないので、片方だけにすると
 * その晩の配信でアラートが絵に戻る。`shapeFull` が
 * `videos.alert?.url ?? 旧 videoUrl` で埋めるので、**旧い欄しか持って
 * いない人も、移した人も、同じように動く。**
 *
 * ## 名前は、誰にでも見せるものではない
 *
 ...
 * 動画1本の上限。
 *
 * 本番でいま使っている1本が **1,053,735 バイト**（実測。528×960・8.6秒）。
 * その4倍を上限にしてある。
 *
