 ...
 * @return {Promise<{count: number; bytes: number}>} 件数と合計バイト数
 */
async function tally(b: PurgeBucket): Promise<{
  count: number;
  bytes: number;
}> {
  const [files] = await b.getFiles();
...
async function survey(b: PurgeBucket, perms: string[]): Promise<Surveyed> {
  ...
  try {
    ...
    return {...base, count: null, bytes: null, listed: "denied", why: kindOf(e)};
  }
}
