...
**例外を人が覚えない。** 「これは あやと本人のIDだから」を毎回思い出す形に
すると、覚えていられるうちはよくても、そのうち見落としになる
（2026-09-13 は、その脇に並んでいた視聴者さん約25人ぶんを危うく見落とした）。
落としてよいものは**道具の側に落とさせる。** いま落としているのは2つだけ
——オーナーの GitHub の名乗りと、オーナー自身のチャンネルID
（`tools/logident.py` の `OWNER_HANDLES` と `OWNER_CHANNEL_ID_ENV`）。

ログの落としかたは `mcp__github__get_job_logs` を `return_content: false` で呼ぶと
署名付きの URL が返るので、`curl` で手元に落として数える（本文を文脈に載せない）。

**数える前に、オーナーのぶんを差す。** 差さないとそのぶんが数に残って
「全ジョブ 0」にならない（未設定なら**数える**側に倒してある。除きすぎるより
数えすぎるほうがまし）。値は画面に出さずに入れる:

```bash
export YOUTUBE_CHANNEL_ID="$(grep -ohm1 -E 'YOUTUBE_CHANNEL_ID: *UC[A-Za-z0-9_-]{22}' \
  <落としたログ> | grep -oE 'UC[A-Za-z0-9_-]{22}')"
```

取り出しているのは **Actions が echo した `env:` の行そのもの**なので、
ここで他人の ID を拾うことはない。取れなければ空になり、そのぶんは数に残る。

> なぜ出るのか: Python ではなく **Actions 自身**が step ごとに env の一覧を
> echo している。secrets へ移せば Actions が伏せるが、#298 であやとの答えは
> 「varibles で良い」——**移さない。これからも出る。**
> 2026-09-20 の回の実測は discover ジョブに7件で、**7件とも同じこの1つ**
> （ほかの3ジョブは 0）。差して数えると4ジョブとも 0 になる。
