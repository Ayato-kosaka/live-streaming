 ...
 * 遠目に「あの国だ」と分かればいい。
 *
 * **id の付けかた**（`Icon.tsx` と同じ決めごと）。
 * 抜き型（`cm-*`）と角丸の型（`fc-*`）は id で呼んでいる。id は文書ぜんぶで
 * 1つの名前空間なので、同じ国旗を2つ置くと id がぶつかる。
 * ぶつかっても困らないのは、**同じ id のものは中身も同じ**だから。
 * 文書順の1つ目に解決されても、その1つ目が消えて2つ目に解決し直されても、
 * 出てくる形は変わらない。**この形を崩さない。**
 * 型の中から `<use href="#別のid">` で他の要素を借りると、
 * 借りた先が消えた瞬間に絵が黙って変わる。
 */
...
const FLAGS: Record<string, Spec> = {
  ...
  cyprus: {
    ...
    paths: [
      ...
      {
        d: `M6.6 9.4c.5-1.3 1.9-2 3.6-2.1 2-.1 3.3.3 5.1.2 1.6-.1 2.7-.5 4.3-.4 1.6.1 2.7.5 3.6 1.3.6.5.4 1.3-.5 1.6-.9.3-1.5.5-2.3 1-1.4.8-3 1.3-4.9 1.4-2.6.1-4.9-.4-6.3-1.2-1.2-.6-2.2-.5-3.5-.7-1.5-.2-2.6-.6-3.1-1.1z`,
        c: "#d57800",
      },
      // カルパス半島。北東へ伸びる尾。これが有るかどうかで、あの島だと分かる
      { d: `M21.3 8.2 26 6.3c.7-.3 1.2.5.6 1l-4.1 2.6z`, c: "#d57800" },
      // オリーブの枝。2本を下で交差させる。
