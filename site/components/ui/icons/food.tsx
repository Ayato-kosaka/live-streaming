...
export const food: Record<string, Draw> = {
   ...
   * 包丁。
   *
   * 前の絵は刃をいちばん明るい灰（`gy`）で塗っていたので、
   * **クリームの紙の上で刃が消えて、茶色い柄だけが浮いていた。**
   * 地の明るさに関係なく形が残るように、刃の**下地を濃い灰**にして、
   * 研いだ面（上）と刃先の線（下）だけを明るく置く。3層で牛刀の断面になる。
   */
  knife: (c) => (
    <>
   ...
   * 箸。
   *
   * 前の絵は**細い棒が2本、先を上に向けて立っている**だけで、
   * つまんでいる玉は手元の側に落ちていた。持つ向きと逆なので何をしているか読めず、
   * 16px では2本の線が消えて料理の並びでいちばん弱かった。
   *
   * 直したのは3つ。**先を左下へ向ける**（つまむ手の向き）／
   * **持ち手 6.8px・先 3px まで太らせる**／**つまんでいるものを先の間に置く。**
   * 角は `strokeLinejoin` で丸めている。四角い角を残さない決まりのため。
   */
  chopsticks: (c) => (
    <>
      <Sh c={c} cy={56} rx={19} ry={3.4} />
      {/* つまんでいるもの。先より先に描いて、箸が手前に重なるようにする */}
      <circle cx="16" cy="48" r="9" fill={c.gr} />
      <path d="M16 39a9 9 0 0 1 0 18z" fill={c.grd} />
      <Gl c={c} cx={12.4} cy={44} rx={3} ry={1.9} r={-24} o={0.6} />
      {/* 奥の1本。手前より暗くして2本に見せる */}
      <path
        d="M52.7 9.5 57.3 14.5 22 45.1 20 42.9z"
        fill={c.wod}
        stroke={c.wod}
        strokeWidth="1.8"
      ...
      {/* 手前の1本 */}
      <path
        d="M56.1 19.2 59.9 24.8 23.8 47.2 22.2 44.8z"
        fill={c.wo}
        stroke={c.wo}
        strokeWidth="1.8"
        strokeLinejoin="round"
      />
      <Gl c={c} cx={47} cy={26} rx={7} ry={1.2} r={-33} o={0.45} />
    </>
  ),
  ...
  eat: (c) => (
    <>
      ...
      {/* 皿は白ではなくクリーム。白い皿に銀の道具だと、
          小さくしたときに全部が同じ明るさになって塊に見える */}
      <circle cx="32" cy="30.6" r="19.5" fill={c.crd} />
      <circle cx="32" cy="29.8" r="19.5" fill={c.cr} />
      {/* フォーク */}
      <path
        d="M17 10h3v10h2.2V10h3v10h2.2V10h3v13c0 3.4-1.6 5.6-4 6.4V51a2.6 2.6 0 0 1-5.2 0V29.4c-2.4-.8-4-3-4-6.4z"
        fill={c.gyd}
      />
      <path d="M26.4 10h3.8v13c0 3.4-1.6 5.6-4 6.4V51a2.6 2.6 0 0 1-2.6 2.6V10z" fill={c.nv} opacity="0.45" />
      {/* ナイフ */}
      <path d="M44 10c4.6 1.8 7 8 7 15 0 5.6-1.8 9.6-4.4 11.4V51a2.6 2.6 0 0 1-5.2 0V10z" fill={c.gyd} />
      <path d="M46.4 12c2.8 3 4.2 7.8 4.2 13 0 5.6-1.8 9.6-4.4 11.4V51a2.6 2.6 0 0 1-2.6 2.6V10.6z" fill={c.nv} opacity="0.45" />
