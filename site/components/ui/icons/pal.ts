...
export const C: Pal = {
   ...
   * 白いものの陰。
   *
   * `#dfe5e9` は白から 12% しか落ちていなくて、**生成りの紙の上で消えていた。**
   * 卵・コーヒーカップ・ワイングラス・塩・スープの器は、どれも白い面と
   * この陰だけでできているので、紙に置くと形が丸ごと無くなって、
   * 中の黄身や赤ワインだけが浮いていた（`/design` の明るい帯で測った）。
   *
   * クリームのもの（`cr` + `crd`）は陰が 30% 落ちていて読めている。
   * 白にも同じだけ落とす。落とす向きは青。白い物の陰は空を映して青くなる
   * （`docs/ac-reference.md` 3章。雪もそう描いてある）。
   */
  wd: "#cfdae7",
  cr: "#fcefd2",
  crd: "#e6cfa2",
