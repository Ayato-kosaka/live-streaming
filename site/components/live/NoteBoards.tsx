 ...
 *
 * ## **この部品は、もう `/board` に出ていない（#160）**
 *
 * 宛先が正式な欄（`islandNotes.theme`）になったので、掲示板は
 * `components/live/Notes.tsx` に替わった。ここを残してあるのは、
 * `components/isle/`（章の島の中の掲示板）が `NotePlace` の型を
 * まだ使っているため。**そちらは担当が別なので手を出していない。**
 *
 * 下の `TAG`（本文の頭の `【国名】`）で仕分ける仕組みは、島の中の掲示板に
 * まだ生きている。既存8件の移行（#162）が済んだら、あちらも宛先で引く形にして、
 * このファイルごと消せる。
 *
 * ## なぜ要るか
 *
 * 視聴者さんが**字を書ける**入り口は、2026年9月の時点で11か所ある。
 * `/next` の企画ごとの付箋が2（**企画の数だけ増える**）、`/board` の掲示板が1、
 * `/nordic` の「この旅に、言う」が1、`/nordic/[国]` が6、`/next/new` の下書きが1。
 * 入れ物のほうは3つ（`islandNotes` `islandIdeas` `islandDrafts`）しかないのに、
 * **書いたものは、書いた場所にしか出ない。** だから貼った本人も、
 * 読むあやとも、いま何が来ているのかを一望できない。
 * 「散らばりすぎてて、一括で見ることができない」（オーナー）。
 *
 * 押すだけの意思表示（今夜のおたずね・北欧のわかれ道・さんせい）は、
 * ここには集めない。あちらは字ではなく数で、棚に混ぜると
 * 付箋の板に棒グラフが1本立つことになる。
 *
 * ## カテゴリーを「貼り先」にした理由
 *
 ...
 *
 * **前後の空白を食わせてある。** 札は機械が付けるが、書いた人が
 * 手で `【 スウェーデン 】` と打ち直すことがある。空白を残すと、
 * 同じ国の棚が2つ並ぶ（実際に試して並んだ）。
 * `【` そのものを半角で打たれたぶんは札として読まない。
 * その1行は下の提案の並びに全文で出るので、消えはしない。
 *
 * **同じ形を `Board.tsx` も見ている。変えるときは両方。**
 */
