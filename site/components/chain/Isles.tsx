...
export default function Isles({ isles }: { isles: AtlasIsle[] }) {
  ...
  /* **`today` が null のあいだは `new Date()` に落とさない。**
     ...
     **モジュールのいちばん外で `chapterNow()` を呼んだ値も、焼いた答えではない。**
     ブラウザでは読み込んだ瞬間の時計で数え直されるので、焼いた HTML が
     「コーカサス周遊・440日」でも、出発の時刻を過ぎたブラウザは最初の描画から
     「北欧周遊・1日」になる。字が食い違って水あわせが落ちた（出発の 19:30Z を境に、
     1分刻みで確認）。**焼いた時刻は `BUILT_AT` から取る**——あれは
     `NEXT_PUBLIC_BUILT_AT` から来るので、サーバ側とブラウザ側で必ず同じ値になる。 */
