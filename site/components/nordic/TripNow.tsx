...
/**
 * 数が届くまでの1行。**場所だけ先に取って、中身の形をした薄い板を置く**
 ...
 *
 * **ここに字を置かない**（`docs/island-misses.md` #145）。ここは「まだ分からない」
 * ときの姿で、焼いた HTML はそのまま何日でも読まれる。「数えています」と書いて
 * あったので、**旅が終わったあとに開いても「ストックホルムまで 数えています」**
 * と言っていた。分かっていないことを字で言うと、その字がいつ読まれるかを
 * こちらが決められない。板なら、どの日に読んでも「まだ出ていない」としか言わない。
 */
function Counting() {
  return (
    <span className="tnow-count-n is-wait" aria-hidden>
      <em className="tnow-ghost">
        <b>0</b>
      </em>
      {/* 板は写しより低くしておく。高いとこちらが行の背を決めてしまう
          （狭い画面では数の字が clamp の下限 26px まで落ちる）。 */}
      <span className="tnow-skel" />
    </span>
  );
}
...
export default function TripNow({
   ...
   *
   * **画面が出るまでは「焼いた日」で決める**（`departed` と同じ形）。`today` は
   * 画面が出てから入るので、`today` だけで決めていたあいだ、**焼いた HTML では
   * `over` がいつでも false** だった。JS が動いたあとの道は `#530` で閉じたのに、
   * 出るまでのあいだの道が開いたままで、旅が終わったあとに開いても
   * 「まだ旅の途中」の札が出る（`docs/island-misses.md` #145）。
   * **1つの部品に「画面が出てから入る値」が2つあるなら、両方の枝を閉じる。**
   */
  const over = planOver(today ?? tripDate(BUILT_AT), until);
   ...
   * どちらも無く旅程からも引けなければ、街の名前は出さない。
   */
  const fromPlan = !place && !arrivedDay && at != null;
  /**
   * 「いま」に出す街の名前。**分からなければ `null`。**
   *
   * ここは `?? "移動中"` だった。いる場所は画面が出てから読むので、
   * **焼いた HTML はいつでも「移動中」**で、旅が終わって半年たった日に開いても
   * そう書いてある（`docs/island-misses.md` #145。#143 の決めごと1 が名指しで
   * 「置かない」と書いている語）。走っているのか、宿で寝ているのか、
   * こちらは知らない。**知らないことを字で言わずに、板で場所だけ取る**
      ...
      <div className={`tnow-pair${idx == null ? " is-wait" : ""}`}>
        <div className={`tnow-at${fromPlan ? " is-plan" : ""}`}>
          ...
          {/* 着いたあとも、いる街は動く（友だちの家に約1週間）。
              ...
              **名前が読めていない日は、板で場所だけ取る。** 字で埋めると、
              焼いた HTML がその字をずっと言い続ける（#145）。 */}
          <b>{nowName ?? <span className="tnow-skel is-name" />}</b>
          <em>{nowCountry}</em>
        </div>
