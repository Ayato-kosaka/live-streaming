 ...
 *
 * **`<` で合っている。** 配られる数は「いま走っている区間」なので、それより前の
 * 区間だけが越えたもの。リガに立っている時点で、ヴィリニュス→リガはもう決まって
 * いて、リガ→タリンはこれから。`<=` にすると、まだ走っていない日の問いまで閉じる。
 *
 * 街にいる日（休息日・まだ発っていない日）は、その街を発つ区間の**半歩手前**が
 * 配られる（`here.ts`）。だから同じ `<` のまま、休息日の問いはその日のうちは開き、
 * 翌朝に発てば閉じる。
 */
...
export function Answer({ leg, seq, fork }: Props) {
  ...
  /* **誰も答えていない日に「聞いてみたら」と言わない。** 数は読めていて 0 だった、
     という日はふつうにある（`counts` が空で返る）。そこを素通しにすると
     「聞いてみたら、答えは半分に割れました」と出て、**誰も押していないのに
     押された話をする**ことになる（`docs/island-standards.md` 10）。 */
