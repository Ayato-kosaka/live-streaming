...
import { travelNow } from "@/lib/stay";
 ...
 *   「1日目」                       … トップに1度も出てこない
 *
 * だったので、**1画面目に、いま何日目でどこにいるかを置く。**
 * 「今日の島」は消さない。あれは残したまま、日付を持つ字をもう1枚、先に出す。
 *
 * ## 島の上ではなく、**島のすぐ下**に立つ
 *
 * 前は看板の下（`.hero-copy` の中＝島の上）にいた。島が札を置くときに避ける箱が
 * そこだから、という理由だったが、**避けるのと、押せるのは別だった。**
 * 幅390 の引き（島ぜんぶ）で、この板は島の北のはしに建つ「伝説の企画」の
 * 当たりをまるごと覆い、島は 48px を取れずにその入口を引っ込めていた。
 * 板を細くしても短くしても解けない（島は絵で、建物は動かせない）。
 * いまは島の外（`app/page.tsx` の `.isle-under`）に立つので、島の入口を1つも
 * 食わない。1画面目に出ることは変わっていない（島の下ふちのすぐ下）。
 *
 * ## 焼き込みと、どう付き合うか
 *
...
export default function TripToday({ slug, days }: { slug: string; days: TripDay[] }) {
  ...
  // 旅に出ていない日は、この板ごと出ない。出発前も、その1ヶ月後も
  if (!trip) return null;

  /**
   * 旅程表の、きょうの行。**無ければ板ごと出さない。**
   *
   * `travelNow()` の終わりは見立て（`plannedDays`）で、**1日ぶん長めに出る**
   * （`content/chapters.ts` の `ended`。短いより長いほうが安全、という決めごと）。
   * 旅程表はその旅の終わる日を切符で知っているので、こちらのほうが細かい。
   * **細かいほうに合わせて、旅程が切れた日からは黙る。**
   * 合わせていないと、ストックホルムを発った翌日の朝に
   * 「いま、旅のとちゅう / 旅に出て 17日目」と出る（時計を進めて実際に出た）。
   *
   * 日付は旅をしている土地の暦で切る（`tripDate`）。日本時間で切ると、
   * 日本の夜のあいだじゅう翌日の行が出る。
   */
