 ...
 *   これまで … 棚の格子。数を持った6マス（Shelf）と、カードへの板1枚
...
export default function Home() {
  ...
  return (
    <main>
      ...
      {/* 島のすぐ下。**背の伸びる板は、島の上に浮かせずにここへ出す。**
          ---------------------------------------------------------
          島は「押せる建物が12軒建っている絵」なので、その上に板を重ねると
          重ねたぶんだけ入口が消える。実測（390px）で、旅の板が引きの
          「伝説の企画」を1軒まるごと押せなくしていて、「今日の島」を開くと
          寄りで7軒・引きで8軒の入口が板の下に入った。
          **板を島の外へ出せば、避ける工夫が要らなくなる。**

          ここに出るのは2つ。
            ・「今日の島」の開いた中身（島の下ふちの札から `#today-drawer` へ）
            ・旅のしるべ（`TripToday`）
          どちらも島の下ふちの札とつながって見えるように、島に接して置く
          （`app/css/today.css` の `.isle-under`）。 */}
      <div className="isle-under">
        {/* 「今日の島」を開いたときの行き先。札は島の下ふちに残る
            （`components/today/Today.tsx` が中身だけをここへ送る） */}
        <div id="today-drawer" />
        {/* 旅のとちゅうだけ出る、いまいる日の札。
      ...
      <div className="page home">
        ...
        >
          <Shelf />
          {/* あやと島カードへの、島からの1本（#448 の続き）。
              ------------------------------------------------
              **もらいかたを出しても、島から行く道が1本も無かった。**
              実測（2026-09-16、本番）で `/` に `href="/cards"` が0件で、
              カードに気づけるのは `/all` の索引か、`/about`・`/friends`・`/me` を
              経由した人だけだった。あやとの「視聴者さん向けに、島の画面へ置く」は
              ここのこと。

              **なぜ「これまで」に置いたか。** カードの面に並んでいるのは
              配信のあった日の写真で、行き先の中身はこの章の問い
              （どこへ行って何を作ったんだろう）の答えそのもの。
              「見にいく」は外へ出る口が並ぶ帯なので、島の中の行き先を
              混ぜると出ていく列に内側の行き先が1つ挟まる。

              **なぜ棚の7マス目にしなかったか。** 棚は2列の格子で、
              7マス目は必ず片側が空く。棚が数えているのは「何がどれだけあるか」で、
              カードは数ではない。だから棚の外に、押せる板1枚で出す
              （`docs/island-design.md` 4章・掲示板の誘いと同じ形）。

              **島の建物は増やしていない。** 島の上の入口は6つのまま
              （`docs/island-design.md` 6章）。

              字は面のリード（`app/cards/page.tsx`）と `CardStrip` と同じ1文を使う。
          ...
          <Link className="hjoin" href="/cards">
            {/* 写真は焼いたスプライトに無い。**大きく出す物はスプライト**だが、
                ...
                寸法は `.hjoin-art` の 72px に合わせて渡す——CSS 側の幅と
                食い違う数を書くと、片方を変えたときに黙ってずれる（#102）。 */}
            <Icon name="photo" size={72} className="hjoin-art" />
            <span className="hjoin-body">
              <b>あやと島カード</b>
