...
export const metadata: Metadata = {
  ...
  /* 距離は `HITCH_KM` から出す。**手で書かない。**
     ...
     **日付と、これから起きる言い方を書かない。** ここは `<meta>` なので、
     次にビルドするまで直らない（`app/layout.tsx` の description と同じ理由）。
     「2026年9月11日出発、9月20日ストックホルム着、27日に発つまで」「北上します」と
     書いてあって、**9月27日にまるごと嘘になる字**だった。旅の日付は面の中に
     `DEPART` / `ARRIVE` / `LEAVE` から出ているので、ここで言い直さない。 */
  description: `スウェーデンに、会いたい人がいます。スウェーデン行きの飛行機が高いので、安いポーランド行きで飛んで、そこから${HITCH_KM.toLocaleString()}kmは人の車で北へ。旅のよてい、ルート地図、国ごとの見どころ、旅のしおり。`,
};
 ...
 *   4''. 9日ぜんぶを1本で   … **9日間のふりかえり**（`/nordic/review`）。
 *                              配信で見せた資料。日ごとの面が「その1日」を見せるのに
 *                              対して、こちらは9日をまとめたもの。**細かいほうから
 *                              粗いほうへ**並べるので、旅程表・ショートの次に置く
...
export default async function NordicPage() {
  ...
  return (
    <PageShell current="next" crumbs={[{ label: "これから", href: "/next" }, { label: "北欧ヒッチハイク" }]}>
      ...
      {/* 9日間のふりかえり。**旅程表とショートのすぐ下。**
          日ごとの面が「その1日」を見せるのに対して、これは「9日ぜんぶ」を
          1本にしたもの。並びは、細かいほうから粗いほうへ。

          **`<Link>` ではなく `<a>`。** 行き先は Next の外（`public/nordic/review.html`）に
          あって、Firebase が配っている。`<Link>` だと道の無い先へ画面の中だけで
          飛ぼうとして落ちる。

          **横向きで見てね、と先に言う。** あれは配信の画面（1920x1080）に出すために
          作ったもので、縦のスマホでは開いた先が「横にしてください」の1枚になる。
          押す前に分かっているほうが、面を1つ余計に踏まずに済む。 */}
      <a className="tile" href="/nordic/review" style={{ marginBottom: "var(--sp-6)" }}>
        <span className="tile-mark">
          <Icon name="theater" size={26} />
        </span>
        <span className="tile-text">
          <b>9日間のふりかえり</b>
          <i>配信で見せた資料。日ごとの道のりと、止まってくれた車と、クイズ。横向きで見てね</i>
        </span>
