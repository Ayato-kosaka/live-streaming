...
/* 北欧旅の節目の日。**`until` と文の両方に同じ日を手で書かない。**
   同じ日が2か所にあると、便が変わったときに片方だけ古くなる
   （`docs/island-standards.md` 8）。正本は `content/nordic.ts` の
   `ARRIVE` / `LEAVE.date` だが、あちらを読むと旅程の表 54KB が
   この面に付いてくるので、写しをここに1つだけ置いて、文はここから作る。 */
const NORDIC_REACH = "2026-09-20";
const NORDIC_UNTIL = "2026-09-27";

/** 「2026-09-27」→「9月27日」。**企画の文の中で、日を手で書かないため。** */
...
export const PLANS: Plan[] = [
  {
    ...
    when: "2026年9月6日(日)",
    date: "2026-09-06",
    /* **「行ってきます」と書かない。** 企画の札は終わったあとも
       「行ってきた」として残るので、日が過ぎたら嘘になる言い方を入れない。 */
    note: "トビリシの山の上の公園でやる、フード＆ワインのお祭り。",
    tags: ["ジョージア", "祭り"],
    place: {
      name: "ムタツミンダ公園",
      area: "トビリシ・ムタツミンダ山の上",
     ...
     9月11日を過ぎたら動かなくなる数なので、ここでは焼いてある。 */
  {
    ...
    when: "2026年9月11日(金)",
    date: "2026-09-11",
    note: "1年住んだジョージアと、この日でお別れ。トビリシからクタイシへ出て、そのまま北欧へ。",
    tags: ["ジョージア", "お別れ"],
    place: {
      name: "トビリシ",
      area: "ジョージア",
    ...
    about: [
      "2025年7月19日にトビリシへ着いてから、384日をこの国で過ごした（途中、35日だけ離れている）。歩いたどの国よりも長い。",
      /* 数は `content/countryStats.ts` の georgia と合わせてある（2026-09-16 に焼き直したぶん）。
         **「17カ国のどこよりも」は書かない。** 国は増えるので、書いた翌月から嘘になる */
      "配信は389本、来てくれた人は1,273人、書き込みは67,047。いちばん長くいた国が、いちばん喋った国になった。",
      "ここでクッキング配信が定着して、クタイシから「なに食べよ」をリリースした。トビリシ・クタイシ・ズグディディ・メスティア・カズベキ・ボルジョミ・バトゥミと歩いた。",
      /* **これから起きる言い方で書かない。** ここは「出る」「最後の日になる」で
         書いてあって、**9月11日を過ぎた日に読むと嘘**だった。企画の札は
         過ぎたあとも「行ってきた」として残るので、いつ読んでも本当な形にする。 */
      "9月11日、トビリシからクタイシへマシュルートカ。その夜の便でポーランドへ飛ぶ、ジョージア最後の日。",
    ],
  ...
  },
  {
    ...
    when: "2026年9月11日(金)",
    date: "2026-09-11",
    note: "関西国際空港から日本を出た日から、ちょうど2年。その日の夜が、北欧への出発。",
    tags: ["周年", "節目"],
    about: [
      ...
      "そしてこの日の23時30分、クタイシから北欧へ。2年目の最後の日が、そのまま3年目の初日。",
    ],
    asks: [
  ...
  },
  {
    ...
    reached: {
      when: NORDIC_REACH,
      /* **着いてからしか出ない字**（`components/live/PlanCard.tsx` の `Reached` は
         `reached.on` が入るまで出さない）。「着く」と書くと、着いた人の画面に
         これから着くように出る。 */
      label: "ストックホルムに着いた",
      /* **「27日に発ちます」と書かない。** 島の様子から「着いた」が届いた日に
         出る字だが、`done` が入るのが遅れれば発ったあとも出る。
         日は上の写しから出して、言い方は日が過ぎても本当なものにする。 */
      say: `会いたい人のいる街に着きました。ここから7泊、${md(NORDIC_UNTIL)}までストックホルム。`,
      fromState: "nordic",
    },
    ...
    about: [
      ...
      `9月12日にカトヴィツェへ降りて、ワルシャワ、ビャウィストク、ヴィリニュス、リガ、タリンと北へ。ヴィリニュスで1日休んで、タリンからは船でヘルシンキ。トゥルクまでヒッチハイクして、夜行フェリーで${md(NORDIC_REACH)}の朝にストックホルム。`,
      `着いて終わりではない。友だちの家に7泊して、${md(NORDIC_UNTIL)}にストックホルムを発つまでが北欧旅。`,
    ],
    asks: [
      /* **日付を書いた問いを置かない。** ここは「9月15日の休息日、ヴィリニュスで
         何をするか」で、**その日を過ぎたら答えようのない問い**が企画の札に残っていた。
         聞くことは、旅のあいだずっと答えられるものにする。 */
      "乗せてくれた人と、なにを話すか",
