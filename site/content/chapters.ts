...
export const CHAPTERS: Chapter[] = [
  ...
  {
    slug: "nordic",
    /* **始まりは前の章の翌日。** コーカサスが 2026-09-11（クタイシを発った日）で
       閉じているので、北欧は 09-12 から。出発の便は 09-11 23:30 に出て
       09-12 01:05 に着くので、旅の時間のほとんどは 09-12 の側にある。

       **終わりはストックホルムを発つ日。** 着いた日（09-20）で閉じると、
       9/21〜9/27 のストックホルム滞在7日ぶんが、どの章にも入らなくなる
       （章は前の `to` の翌日から始まる並びなので、切れ目に落ちる）。
       あやと 2026-09-06「ストックホルム出るまでが北欧旅です」（`content/nordic.ts:1046`）、
       および GitHub #282 の「案B」。 */
    name: "北欧周遊",
    from: "2026-09-12",
    to: "2026-09-27",
    countries: ["poland", "lithuania", "latvia", "estonia", "finland", "sweden"],
    ...
    // **`from` が入っても消さない。** 上の `opensAt` の説明は「実際に始まったら
    // 消す」と書いてあるが、いまはこれを見ているところが2つある——
    // `app/island/[chapter]/page.tsx` が「素材が1つも無い島」を北欧旅の spec で
    // 受け止める最後の網に使っていて（`bare && c.opensAt`）、
    // `python/nordic_depart.py` は出発の日をここからしか引けない。
    // `began()` は `from` を先に見るので、置いておいても日付は動かない。
    opensAt: "2026-09-11T23:30:00+04:00",
    // クタイシ→カトヴィツェ→ワルシャワ→ビャウィストク→ヴィリニュス（休息1日）→
    // リガ（休息1日）→タリン→ヘルシンキ→ストックホルム。9月20日に着いて、そこから
