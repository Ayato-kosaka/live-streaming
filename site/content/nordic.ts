...
export const ROUTE: Leg[] = [
  {
    ...
    time: "3時間35分",
    date: "2026-09-11",
    fixed: "Wizz Air W6 1200 / 23:30 発 → 翌 01:05 着",
    note: "唯一の飛行機。機内持ち込みのカバンひとつだけで乗る。",
    enters: "poland",
    fare: { what: "クタイシ→カトヴィツェの飛行機", yen: 13000, src: "219ラリ" },
    fork: {
      q: "着くのは深夜1時。始発が動きだすまで、空港で数時間あります。",
      options: [
  ...
  },
  {
    ...
    note: "アウシュヴィッツ強制収容所の跡地。日帰りで行って、クラクフに戻る。",
    side: true,
    art: "rails",
  },
  {
    ...
    fare: { what: "ワルシャワの一泊" },
    fork: {
      q: "乗せてもらえるまでに、何回も断られます。その数を数えますか。",
      options: [
  ...
  },
  {
    ...
    enters: "latvia",
    // 十字架の丘（シャウレイ）は幹線から少し外れる。寄るかどうかは下のわかれ道しだいなので、
    // 「通る街」ではなく「寄るかもしれない街」に置く。
    maybe: ["シャウレイ"],
    fare: { what: "リガの一泊" },
    fork: {
      q: "途中のシャウレイに、十字架が20万本立つ丘があります。",
      options: [
  ...
  },
  {
    ...
    km: 307,
    time: "1日",
    note: "バルト海沿いのVia Baltica。濡れながら立つ日が必ずある。",
    enters: "estonia",
    fare: { what: "タリンの一泊" },
    fork: {
      q: "9月のバルトは3日に1日は雨。降られたら、どうしますか。",
      options: [
  ...
  },
  {
    ...
    km: 80,
    time: "2時間",
    note: "1日10〜13便のフェリー。ここだけは親指では渡れない。",
    enters: "finland",
  ...
  },
  {
    ...
    move: "ferry",
    time: "17時間（船中泊）",
    note: "夜に出て、朝に着く。ヘルシンキでは泊まらない。",
    enters: "sweden",
    fare: { what: "夜行フェリーの席と船室", yen: 20300, src: "€43＋船室€75.50" },
    fork: {
      q: "17時間の夜行フェリー。着いた朝が、この旅の終点です。",
      options: [
 ...
 * 3日目ビャウィストク、4日目ヴィリニュス、5日目リガ、6日目タリン、
 * 7日目はヘルシンキ経由でストックホルム。**ここまでが本人の言葉**
 * （2026-09-05 に4日目から7日目まで言い切った）。あとは予備の2日で、ぜんぶで9日。
 *
 * **分からない日を埋めない。** いまは全部の日が埋まっているが、
 * 埋まっていない日ができたときに勝手に埋めないこと。埋めると旅程表ぜんぶが
 * 「たぶんこうだろう」に見えて、決まっている日まで信じられなくなる。
 *
...
export type Day = {
   ...
   *
   * 旅程表の行は「どこからどこへ」しか言えない。それだけだと、9行が
   * 全部おなじ形の行に見えて、押す理由がどこにも無い。
   *
...
export const DAYS: Day[] = [
  {
    ...
    label: "出発",
    date: "2026-09-11",
    lead: "この旅で唯一の飛行機。夜のクタイシを出て、真夜中のポーランドに降りる。",
  ...
  },
  {
    ...
    n: 1,
    date: "2026-09-12",
    lead: "深夜1時のカトヴィツェから、その日のうちにアウシュヴィッツまで。",
  ...
  },
  {
    id: "day-2",
    n: 2,
    lead: "ポーランドを縦に295km。壊されて建て直された街のほうへ。",
    legs: [leg("krakow-warszawa")],
    stay: "ワルシャワ",
  },
  {
    id: "day-3",
    n: 3,
    lead: "ヴィリニュスまでの513kmは1日で抜けられない。その半分の200kmだけ進む。",
    legs: [leg("warszawa-bialystok")],
    stay: "ビャウィストク",
  },
  /* **4日目から7日目まで、あやとが日を言い切った**（2026-09-05）。
     前はここに「4日目・5日目はまだ決めていません」の行と、
     「リガのあと（何日目になるかは進み方しだい）」の行があった。
     どちらも本人が日を決めたので消してある。**戻さないこと。**
     7日目にストックホルムへ着いて、予備2日を足して9日。 */
  {
    id: "day-4",
    n: 4,
    lead: "オグロドニキの国境を越えてリトアニアへ。車がいちばん少ない区間で、最初の山場。",
  ...
  },
  {
    id: "day-5",
    n: 5,
    lead: "293kmでリガまで。途中のシャウレイに、十字架が20万本立つ丘がある。",
  ...
  },
  {
    id: "day-6",
    n: 6,
    lead: "バルト海沿いのVia Balticaを307km。9月は3日に1日は雨が降る。",
  ...
  },
  {
    id: "day-7",
    n: 7,
    lead: "親指では渡れない海を、フェリー2本で越える。着いた朝が終点。",
    // ヘルシンキでは泊まらない。夜行フェリーに乗ってしまうので、
    // タリンを出てからストックホルムに着くまでが1日ぶんの行。
  ...
  },
  {
    ...
    label: "予備の2日",
    bare: true,
    say: "雨で動けない日と、車がつかまらない日のぶん。ぜんぶで9日ぐらいの見立てです。",
  },
];
...
/**
 * 1日ぶんのページを持つ日。**区間のある日だけ。**
 *
 * 予備の2日には道が無い（どこへ行くかが決まっていないから予備なのであって、
 * 中を開いても書けることが1つも無い）。旅程表では行として置いたまま、
 * そこだけ押せない行にしてある。
 */
export const DAY_PAGES: Day[] = DAYS.filter((d) => (d.legs?.length ?? 0) > 0);

export const dayBySlug = (slug: string) => DAY_PAGES.find((d) => daySlug(d) === slug);

/** 「オシフィエンチム（アウシュヴィッツ）」→「オシフィエンチム」。地図と見どころの街名に合わせる。 */
 ...
 *
 * **日ごとの値を持たない。** 日にちが決まっているのは切符のある2日だけで、
 * 残りは乗せてもらえた日でずれる（`Day.date` と同じ決まり）。
 * 9月中旬なら1週間で15分ほどしか動かないので、月の半ばの1つで足りる。
 * 画面でも「9月中旬の◯◯」と断って出す。
 */
export const SUN: Record<string, { rise: string; set: string }> = {
  カトヴィツェ: { rise: "6:20", set: "19:00" },
  クラクフ: { rise: "6:16", set: "18:56" },
  オシフィエンチム: { rise: "6:19", set: "18:59" },
 ...
 * 選ぶのは「何日目か」であって、「どの区間か」ではない。1日に2区間ある日
 * （出発の翌日と、着く日）に、同じ話を2回書かせることにもなる。
 *
