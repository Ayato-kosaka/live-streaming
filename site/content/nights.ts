...
const FIXED = {
   ...
   * 表紙の目印。**島の上の板に出るので、短く。**
   *
   * 390px の板に入る字は、13.5px で 24文字ぶん（実測 325px）。ここを越えると
   * 折り返して、右寄せなので最後の1文字がぽつんと浮く。実際に旅の言い方が
   * 26文字あって、390px で「送」1文字（13px）だけが2行目に落ちていた。
   *
   * 形は `[いつ]、世界のどこかから生放送`。旅かどうかで変わるのは前半だけ。
   * **前半は `bare` と同じものを使う**（別の言い方を1つ増やさない）。
   * 触ったら `node tools/sprites/orphan.mjs` で4幅を測る。
   */
