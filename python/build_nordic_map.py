...
COUNTRY_LABELS = {
    ...
    # トゥルクの名札が画面のこの高さに入ったので、国名は東へ寄せた
    "finland": (28.0, 61.4, 40),
    "sweden": (14.9, 59.9, 40),
}
...
SEA_LABELS = [
    ...
    # トゥルクが増えたので、湾の名前を北西へ逃がした。前の場所（19.5, 60.9）だと
    # 名前の右端が、トゥルクの名札にそのまま重なっていた。
    ("ボスニア湾", 18.3, 61.5, 26, -22),
    # ヘルシンキとタリンの名札のあいだに挟まっていたので、湾の奥（東）へ寄せた
    ("フィンランド湾", 27.6, 59.62, 21, -4),
]
...
def main() -> None:
    ...
    # ---- 街 --------------------------------------------------------
    # 2026-09-06 に旅程が入れ替わった。**クラクフとオシフィエンチムは通らない。**
    # カトヴィツェから直接ワルシャワへ北上して、ヘルシンキからは
    # トゥルクまでヒッチハイクしてから夜行フェリーに乗る。
    cities = [
        ...
        ("turku", "トゥルク", 22.267, 60.452, "pass", "finland"),
    ...
    out["legs"] = [
        ...
        # ここに 80km を出すと、すぐ横のヘルシンキ→トゥルク 165km と札が重なる
        # うえに、そもそも80kmは自分で進む距離ではない。
        leg("tallinn", "helsinki", "ferry", 0.20, None),
        # ヘルシンキ→トゥルクは最後のヒッチハイク。札は線の下（南）へ逃がす。
        # 上はボスニア湾の名前とフィンランドの国名で混んでいる。
        leg("helsinki", "turku", "hitch", 0.10, -52),
        leg("turku", "stockholm", "ferry", 0.14, 40),
    ]
