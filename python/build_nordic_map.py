...
# いちばん南はカトヴィツェ(50.26N)で、その下に 2.4 度、いちばん北の
# トゥルク(60.45N)の上に 1.7 度。下はカトヴィツェの名札と
# 「クタイシから」の飛行機の札が入るぶんだけ余分に要る。
# **クラクフとオシフィエンチムはもう通らない**（2026-09-06 に旅程が変わった）が、
# 範囲はそのままにしてある。切り上げると縮尺が変わって、街の名札の逃がし方
# （`components/nordic/RouteMapSvg.tsx` の LABEL）を全部やり直すことになる。
...
# 置き場所は、**街の名札・金の札・方位磁針をよけたところ**を実際に測って決めた
# （道具は `tools/sprites/nmcover.mjs`）。前の値は、街の名札に
# エストニアが 18%、リトアニアが 11% 食われていた。
# 大きさも下げてある。名前の下に地を敷いた（`RouteMapSvg.tsx` の nm-wash）ので、
# 大きいままだと敷いた地が地図の3割を覆う。
COUNTRY_LABELS = {
    "poland": (17.0, 52.1, 46),
    # 前は (22.0, 55.35)。シャウレイの名札とヴィリニュスの金の札に挟まれ、
    # さらにヒッチハイクの線が名前の上を通っていた。線の西へ出す。
    # ここはシャウレイの名札と 295km の札に上下を挟まれていて、置ける高さが
    # 48 しかない。**枡**（`cellBox`）で当たるので、大きさ 22 まで下げてある。
    "lithuania": (21.655, 55.184, 22),
    # 前は (26.9, 56.9)。シャウレイの金の札が真上から降りてくる場所だった。
    # 南へ下げて、ヴィリニュス〜リガの線のわきを距離の札に空ける。
    "latvia": (26.542, 56.398, 26),
    # 前は (26.2, 58.7)。タリンの名札に 18% 食われていたので、南へ下げた
    "estonia": (26.745, 58.369, 24),
    # 前は (28.0, 61.4)。**方位磁針に 34% 食われていた**（トゥルクをよけて
    # 東へ寄せた名残だが、トゥルクはもう旅程にいない）。西へ戻す
    "finland": (24.219, 61.885, 34),
    # 前は (14.9, 59.9)。ストックホルムの金の札に 42% 食われる場所だった
    "sweden": (13.628, 60.107, 34),
}
...
SEA_LABELS = [
    # 前は (18.5, 56.3)。シャウレイの名札に 15% 食われていたので、西へ逃がす
    ("バルト海", 16.05, 55.988, 34, -14),
    # 前は (18.3, 61.5)。フィンランドの国名を西へ戻したぶん、こちらも西へ寄せる
    ("ボスニア湾", 16.725, 61.667, 26, -22),
    # 湾はタリンとヘルシンキのあいだの細い帯。前（27.6, 59.62）は両方の名札に
    # 食われて 36.6% 隠れていた。**字は画素の形ではなく枡で当たるので**、
    # 名札のあいだに残る高さは 17 しかなく、どう小さくしても入らない。
    # ヘルシンキの名札を西へ返して（`RouteMapSvg.tsx` の LABEL）北側を空け、
    # そのぶん湾の名前を上げてある。
    ("フィンランド湾", 27.89, 60.061, 20, -4),
]
...
RANGES = [
    ...
    # カルパチア（タトラ〜ビェシチャディ）。ポーランド南部の壁。
    # 通らないが、画面の下辺が緑一色になるのをこれで止める。
...
def main() -> None:
    ...
    # ---- 街 --------------------------------------------------------
    # 2026-09-09 に旅程が確定した（life-ops#91）。**クラクフとオシフィエンチムは
    # 通らない。トゥルクも通らない。** カトヴィツェから直接ワルシャワへ北上して、
    # ヘルシンキからはそのまま夜行フェリーでストックホルムへ渡る。
    #
    # トゥルク経由は見送りになった（ドミトリーが €20 以上でフェリーの差額を
    # 相殺してしまう、というのが向こうの判断）。**点を残すと、地図が
    # 通らない街を通ったように見せる。**
    cities = [
        ...
        ("helsinki", "ヘルシンキ", 24.938, 60.170, "stay", "finland"),
    ...
    def leg(a: str, b: str, move: str, bulge: float = 0.10, km_off: float | None = 30.0) -> dict:
        ...
        #
        # **ここで決まるのは「いちばん置きたい場所」までで、最後に置く場所ではない。**
        # 札がぶつかる相手（街の名札・「いま ここ」の金の札）は、位置も大きさも
        # TS の側にしかない。しかも「いま ここ」は**その日いる街にだけ**出るので、
        # 出る場所は17日ぶんある。ここで1点を決め打ちすると、街の名札を1つ動かした
        # 日に黙って下敷きになる（実際に 197km が 93% 隠れていた）。
        #
        # だから**候補を並べて渡す**。線に沿って前後へ、線から左右へ振った点を
        # 「いちばん置きたい場所から遠いほど後ろ」に並べる。選ぶのは、ぶつかる
        # 相手を知っている `components/nordic/RouteMapSvg.tsx`。
        def spot(t: float, off: float) -> tuple[int, int]:
            i = max(0, min(n - 1, int(t * n)))
            j = min(n, i + 1)
            tx, ty = pts[j][0] - pts[i][0], pts[j][1] - pts[i][1]
            tl = math.hypot(tx, ty) or 1.0
            return (round(pts[i][0] - ty / tl * off), round(pts[i][1] + tx / tl * off))

        km_at = None
        km_spots: list[list[int]] = []
        if km_off is not None:
            km_at = list(spot(0.5, km_off))
            cand = []
            for t in (0.5, 0.44, 0.56, 0.38, 0.62, 0.32, 0.68, 0.26, 0.74):
                for off in (km_off, -km_off, km_off * 1.5, -km_off * 1.5,
                            km_off * 0.5, -km_off * 0.5, 0.0):
                    # どれだけ譲ったか。**線に沿って動くのと、線から離れるのを
                    # 同じ重さで数えない。** 距離の札は区間の名前なので、線から
                    # 離れるほど「どの区間の距離か」が読めなくなる。逆に線に
                    # 沿って動くぶんには、同じ区間を指したままでいられる。
                    # 素の距離で数えていたころ、197km が線から 42 離れた——
                    # ワルシャワの南、290km の真上——に出て、**2つとも
                    # カトヴィツェ〜ワルシャワの距離に見えていた**。
                    cost = abs(t - 0.5) * span + abs(off - km_off) * 0.45
                    cand.append((cost, spot(t, off)))
            ...
            for cost, xy in cand:
                if xy in seen:
                    continue
                seen.add(xy)
                # 3つめは「いちばん置きたい場所からどれだけ動くか」。
                # 選ぶ側が、ぶつかりの面積と天秤にかけるのに要る。
                km_spots.append([xy[0], xy[1], round(cost)])
    ...
    out["legs"] = [
        leg("katowice", "warszawa", "hitch", 0.12, 46),
        leg("warszawa", "bialystok", "hitch", 0.10, 40),
        leg("bialystok", "vilnius", "hitch", 0.08, 40),
        leg("vilnius", "siauliai", "side", 0.14, None),
        # 距離の札は線の西側へ。東はラトビアの国名とサローチャイの国境の棒で
        # 埋まっていて、そちらを既定にすると札が線から 127 まで逃げる。
        leg("vilnius", "riga", "hitch", -0.10, -46),
        # 距離の札はリガ湾（西）側へ出す。東はエストニアの国名とタリンの名札で
        # 埋まっていて、そちらに置くと札が線から 117 も離れたところまで逃げる。
        leg("riga", "tallinn", "hitch", -0.10, -44),
        # フェリーには距離の札を出さない。**親指で進む距離だけを地図に書く。**
        # 自分で進んだ距離ではないので、80km も 400km も同じ扱いにする。
        leg("tallinn", "helsinki", "ferry", 0.20, None),
        leg("helsinki", "stockholm", "ferry", 0.14, None),
    ]
