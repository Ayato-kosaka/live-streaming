...
#
# 値段は測った（2026-09-18。この箱）: `functions` の `npm ci` が **10秒 /
# 663 パッケージ**、6本で **90秒**（1本 15秒。**中身ではなく、1本ずつ
# 自分で `tsc` を通すぶん**）。`CARDS_LIB_DIR` を渡せばその 15秒は消えるが、
# あれは「壊した写しで回す」ための口なので、本物の `lib` を指させない
