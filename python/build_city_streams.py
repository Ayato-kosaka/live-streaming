...
**あちらも同じ2つにそろえた**（`docs/island-misses.md` #166）。落としすぎている
...
どちらが本物か読めない。`docs/island-misses.md` #166 の決めごと5）
...
# 守りは2つとも `build_dead_streams` が持つ。**差す側が同じ顔で並べられる**ように、
# 置き場所をそろえてある（`sql_not_in` が取り置き、`sql_public` が非公開）
