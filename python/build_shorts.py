...
def main() -> int:
    ...
    if a.fetch:
        ...
        #
        # **ただし 0 で返さない。** 0 で返すと「今日はショートが出なかった晩」と
        # まったく同じ顔になり、**何週間届いていなくても誰も気づけない**。
        # 凍りの見張りは 0（見ない）にしてある——4ヶ月出ない期間が実際にあるので
        # 日数では狼少年になる——ぶん、**「届いたか」だけは別に言う。**
        # 3 は「届かなかった」専用。呼ぶ側（`rebake.yml`）は、これを
        # **赤ではなく警告**として出し、1（手元のものが消えていた）と分ける。
        r = fetch(a.gap)
        if r == 2:
            print("ショートの一覧に届きませんでした。棚は前のままです（焼いていません）")
            return 3
        if r:
            return 1
