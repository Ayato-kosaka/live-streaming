...
5. **リトライ制御**: 7日間ルールに基づくリトライ（チャットがまだ出ていないだけなら失敗にしない）
...
### チャットがまだ出ていないとき（7日間ルール）

アーカイブのチャットは、配信が終わってすぐに出ないことがある。
**これはエラーではないので、失敗として記録しない。**

- `first_seen_at` から 7日以内 → `WAITING`（次の晩にもう一度）
- `first_seen_at` から 7日以上経過 → `SKIPPED`（永久スキップ）

**24時間で `FAILED` に倒すのはやめた（2026-09-10）。**
ジョブは日に1回しか走らないので、初回確認からの経過は 0h → 約22h → 約46h と飛ぶ。
「24h＋ぶれ3時間 以内なら WAITING」という窓は**1回しか通らない**ので、
2晩目には必ず窓の外にいて `FAILED` になっていた。7日間ルールには一度も届いていない。

### 本物のエラーのとき

yt-dlp の失敗・パース失敗・BigQuery の失敗は、これまでどおり失敗として扱う
（`handle_failure`）。

- `first_seen_at` から 7日以内 → `FAILED`
- `first_seen_at` から 7日以上経過 → `SKIPPED`

### 次に試してよくなる時刻（`next_retry_at`）

`first_seen_at + 24時間 - 3時間`。定時実行は毎晩数十分ぶれ、**前の晩より早く
走ることがある**ので、きっかり24時間後に置くとその晩は拾われずまる1日待つ
（`CRON_JITTER_SECONDS` に実測）。

2晩目より先ではこの値はもう過ぎているが、それで正しい。拾い直すクエリは
`next_retry_at <= CURRENT_TIMESTAMP()` で見るので、過ぎていれば毎晩対象に入る。
...
境目（0h / 22h / 46h / 6日 / 8日）で状態がどうなるかを見ている。
**46h が `WAITING` のまま**であることが要。ここが `FAILED` に変わったら、
7日間ルールにまた届かなくなる。
...
# 定時実行のぶれ（秒）。next_retry_at をこのぶん手前に置く
CRON_JITTER_SECONDS = 3 * 60 * 60  # 3時間
...
- チャットファイル不在 → エラーではなく「まだ出ていない」。7日間 WAITING
