...
## 「拾い直せる」の列は、窓の内側のことしか言っていない

拾い直すクエリ（`python/bq/queries.py`）は `first_seen_at >= 現在 - 7日` の
枠（窓の内側）と、**それより古いぶんを1晩20本まで拾う枠**（窓の外側）の
2つでできている（2026-09-17。`docs/island-misses.md` #123）。

このスクリプトが出す「拾い直せる／7日超」は**窓の内側かどうか**で、
「7日超」と出たものも、窓の外側の枠から順に拾われて `SUCCEEDED` か
`SKIPPED` になる。**そちらは `next_retry_at` を直す必要が無い**
（窓の外側の枠は `next_retry_at` が過ぎているものしか拾わないが、
2晩目以降はどの行ももう過ぎている）。
