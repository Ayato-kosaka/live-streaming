...
## いまも残っている用事は「**ずれている値を直す**」1つだけ（2026-09-18・#158）

**空欄を埋めるのは、もう毎晩の仕事。** `python/doneru_supporters.py` が
毎晩、**書類は在るのに `firstSeenAt` が空**の人をぜんぶ並べて引くように
なった。だから「押さないと入らない」ものは、もう1つも無い。
**この道具を押さなくても、空欄はひとりでに埋まる。**

それでもここを消さないのは、毎晩のほうが**入っている値を絶対に
上書きしないから**。あとの日付で塗ると「来た日」が繰り上がって嘘になるので、
あちらは空欄しか触らない。**すでに入っている値がずれていたときに直せるのは、
ここだけ。** ずれが出るのは2つの場合:

1. 昔の取り込みが `now`（ジョブが走った時刻）を焼き付けたぶん（下の経緯）
2. 寄付の表が作り直されて（#186）、消えていた**もっと古い1回**が戻ったとき。
   毎晩のほうは上書きしないので、古い値のまま残る

**人が押す道具のまま置く。** 上書きは、押した人が結果を見て決めることで、
毎晩ひとりでにやってよいことではない。
