"""旅の写真の実体だけを、退避する／戻せるか確かめる。

**なぜ `backup_now.py` に足さず、1本分けたか。**
あちらは退避を丸ごと1回まわす（Firestore 全部＋BigQuery 4表＋突き合わせ）。
1回で **BigQuery を 490MB 読む。** 写真は1回に 128MB までで区切ってあって、
溜まっている 340MB を流し切るには何回か続けて押すことになる。
そのたびに 490MB を読み直すのは、**取る必要のないものの代金**。
外へ HTTP を叩くのも、上限を押し上げたいのもここだけなので、口を分ける。

ARGS 例:
  {}                                … 下見。**1バイトも書かない**
                                       （何枚あって、何枚取ってあって、何枚残りか）
  {"apply": true}                   … 上限（既定 128MB / 400枚）ぶん取る
...
**既定は書かない。** 外の口を叩くものなので、何枚取りにいくのかを先に出す。
...
def main() -> None:
    ...
    log.info("実体 %s 枚 / 取ってある %s 枚 / この回に取った %s 枚 / 残り %s 枚 / 落ちた %s 枚",
             r.get("n_all"), r.get("have"), r.get("n"), r.get("left"), r.get("failed", 0))
    if r.get("left"):
