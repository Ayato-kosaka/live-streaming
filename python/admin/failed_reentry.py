...
`youtubei/v1/player` で2段目——毎晩 `build_dead_streams.py` が島の名指しぜんぶに
当てているのと**同じ関数**を呼ぶ（`probe_all`）。判定器は増やさない。

**ただし、あの毎晩のぶんは覆いきれない。** あれが見ているのは「島が名指ししている
配信」（2026-09-19 に数えて813本）で、**37本のうち島に出ているのは0本**。
1本もあちらの測りに入らないので、ここは**候補を自分で列挙して、自分で測る。**

  - 0本なのは #160 で非公開の15本を `cityStreams.ts` から落としたから
    （落とす前の版で数えると15本あった）。**焼き込みから落とすほど、
    あちらの目からも外れる**——隠したものは、戻ったことにも気づけなくなる。
    ここが自分で列挙している理由が、そのぶん強くなっている

  - 覆う相手を広げるだけで、**測りかたは借りたまま**
  - 測るのは37本だけ（**実測 0.32秒**。2026-09-19 の走者2回、run 35417891908 /
    35417988198。1段目は8本ずつ並べて当て、**2段目は 200 が返ったものにしか
    当たらない**——いまは37本とも非公開なので0本。書いたときの見込み「2〜3分」は
    2段目に全部当たる前提の数字だった）。37本を毎晩
...
def main() -> int:
    ...
    # かかる時間は**2段目が何本あるか**で決まる。1段目は8本ずつ並べて当てるので、
    # 全部非公開の晩は1秒で終わる（37本で 0.32秒。2026-09-19 の走者で実測）。
    # 見込みを書かないのは、そこが読む人の「待つ目安」になるから（#141 の決めごと6）
    log.info("%d 本を測ります（1段目 oembed → 200 が返ったものだけ2段目 player）",
             len(rows))
