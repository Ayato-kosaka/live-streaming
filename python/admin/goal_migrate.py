...
**毎晩ひとりでに流れている**（`.github/workflows/goal_backup_nightly.yml` が
`goal_backup.py` 越しにここを呼ぶ。#614）。手で流すのは、表を消す前と、
配信の直後にすぐ写しておきたいときだけ。

控えが古くても本番の額はずれない（GAS を先に読むので）。**古いのは、
GAS がこけた瞬間に見える。** そのとき出るのが最後に流した日の額で、
9日前まで開いたことがある（#614）。だから毎晩に繋いだ。
...
合わなければ1行も書かずに `EXIT_MISMATCH`（3）で落ちる。**それ以外の
落ちかた（表が読めない・鍵の形が違う・書いたあとがずれている）は 1。**
毎晩ぶら下げている側は、この2つを別のものとして扱う（3 は数分おいて
もう一度、1 はその場で赤）。
...
# **本番と1円まで合わなかった**ときだけの終了コード。1（ほんとうに壊れている）
# と分けてあるのは、毎晩ぶら下げる側（`goal_backup.py`）が
# 「本番がまだ5分ぶん寝ている」と「見ている表が本番の元ではない」を
# 見分けるため。**止め金そのものは1バイトも変えていない**——どちらの道でも
# 1行も書かないし、手で押したときはどちらも赤で返る。
#
# **0 にしない。** 「合わなかった」を緑で返すと、手で押した人にも
# `run_admin_script` のログにも、合わなかったことが伝わらなくなる。
# 緑にしてよいかは、晩を数えている側（控えが何晩古いか）にしか決められない。
EXIT_MISMATCH = 3
...
# **import しても走り出さない。** 毎晩の口（`goal_backup.py`）が、
# 数分おいて2回目を試すためにこの `main()` をそのまま呼ぶ。
# 手で押したとき（`run_admin_script` から `python goal_migrate.py`）の
# 振る舞いは変わらない。
