...
## 画面はできた。**それでもここを残す**（2026-09-24）

3つとも `/me/desk` の「貯金箱」から入れられるようになった
（`site/components/me/FundDesk.tsx` → `functions/src/fundDesk.ts`）。
ふだんはそちらを使う。**ここは残す。** 理由は3つ。

  - **画面が落ちた日に、額を直す道が1本も無くなる。** Hosting の配りが
    こけても、Functions が 500 を返しても、こちらは Firestore を直に叩く
  - **ログインが要らない。** 画面はあやとの合言葉が要るので、
    入り直せない端末しか無い日に詰む
  - **既定で1バイトも書かない**（`{"apply": true}` が要る）。
    大きいものを入れる前に、書類IDと上書きになるかを下見できる

**書き方は画面と1文字も違わない。** 書類IDの式は `python/fund_box.py` に
あって、画面側（TypeScript）はその写し。ずれていないことは
`functions/selftest/fund_docid_selftest.mjs` が毎 PR で突き合わせる。
...
**ここから入れたぶんは、額にすぐ出ない。** 島の豚も配信の豚も
`island/state.fund.box`（焼き直し）を読んでいて、それを書き直すのは
毎晩の掃除（`python/fund_daily.py`）だから。すぐ出したいときは
`fund_sync` を `{"apply": true}` で流す。

画面（`/me/desk` の「貯金箱」）から入れたときは、口が書いたその場で
焼き直すので、この一手は要らない。
"""
