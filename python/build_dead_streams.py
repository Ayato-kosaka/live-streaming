...
`pick_deep`）。1段目（oembed）は速い（2026-09-19 で 820本・4〜5分）ので毎晩
ぜんぶに当てる。**この数は毎晩ずれる**（島の名指しは焼き直すたびに増える）。
403 / 401 / 404 はそこで出る。
...
# 30日たった晩に島の名指しぜんぶがいっぺんに来て、その晩だけ何十分もかかる。
# 古いものから順に取るので、上限で溢れたぶんは翌晩に回る
# （2026-09-19 の 813本なら 813 ÷ 40 ＝ **21晩**で一周する。30日より短いので追いつく。
#  **ここは本数が増えると詰まる側**——1,200本を超えると一周が30日を追い越すので、
#  そうなったら上限のほうを上げる）
DEEP_RECHECK_MAX = 40

# 1晩に**初めて**深く見る本数の上限。ふつうは増えたぶん（数本）しか来ないが、
# 覚えの JSON が消えた朝はここに島の名指しぜんぶ（2026-09-19 で 813本）が並ぶ。
# **そこで何十分もぶら下がらない。**
...
def pick_deep(live: list[str], deep_seen: dict[str, str], known_bad: set[str], today: str,
    ...
    島の名指しぜんぶ（2026-09-19 で 813本）に当てると何十分もかかって、
    そのうち YouTube に締め出される（719本で 18〜25分の実測から）。
