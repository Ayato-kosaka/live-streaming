...
#
# **ここは半額にしない。貯金箱の額ではないから。** 貯金箱に入るのは半分で、
# それは仕様（`docs/nordic-fund.md` 9.1）。ここが出しているのは「直近365日に
# スパチャがいくらあったか」で、貯金箱とは別の数。
#
# **貯金箱のほうは `python/fund_daily.py` が `fund.box` に書く。**
# 同じ欄を2人で書かないよう、こちらは `fund.superchat` と `fund.people` の
# ままにしてある。
FUND_DAYS = 365
FUND_SQL = f"""
WITH p AS (
