...
**2026-09-18 の実測**（本番で `--dry-run`。run 35394097689）: 対応表 31件 /
`firstSeenAt` が空 **0件**。全員に入っているので、**いまは1本も増えていない。**
別の道（`donors_first_seen.py` の下見。run 35394216547）でも
「すでに合っている 31件 / 空いていた 0件 / 元データに無くて触らない 0件」で、
2つの数え方が揃った。
