...
KEEP = {
    ...
    "islandFundConfig": (
        "Doneru の鍵（#639）。**失うと豚の貯金箱から Doneru のぶんが丸ごと消える。**"
        "取り直すには Doneru の管理画面に入れる人が要る——機械では戻せない"
    ),
    "islandGoal": (
        "**旧。** GAS の `Goals` を写しただけの1書類で、#639 でもう誰も読まない。"
        "**消さずに取る**——鍵の写しが入っていて、戻すときの最後の1本になる"
    ),
...
SKIP = {
    ...
    "islandGoalHealth": (
        "**役目が終わった札**（`last` 1書類）。#614 で毎晩の写しが何晩続けて"
        "書けなかったかを覚えていたが、#639 でその写しごと畳んだ。もう誰も書かない"
    ),
