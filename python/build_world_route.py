...
#   西 グラスゴー -4.25 / 東 アブダビ 54.38 / 南 アブ・シンベル 22.34 / 北 ヘルシンキ 60.17
#
# **北を 60.5 から 70.5 に広げた**（2026-09-22、GitHub #450）。北欧の6カ国を
# 足すときに、60.5 のままだとヘルシンキのピンが**上辺から 13.6px** のところに
# 落ちてバッジが切れた。そこだけ避けて 63.0 にする手もあったが、
# **スウェーデンは 69.1N、フィンランドは 70.1N まで伸びている。**
# 63 で切ると、歩いた国が2つとも地図の途中でちぎれる。半分だけ描かれた国を
# 2つ並べるくらいなら、縦を 1145 → 1645 にして全部入れるほうがいい。
#
# **広げると既存18カ国の見え方も全部動く**（view の高さが変わるので、
# ピンも札も寄せ枠も位置が変わる）。撮り比べて、はみ出し・重なり・切れが
# 出ていないことを見てある。
LON_MIN, LON_MAX = -11.0, 57.5
LAT_MIN, LAT_MAX = 20.0, 70.5
...
ROUTE = {
    ...
    "Armenia": "armenia",
    "Iran": "iran-border",
    # 北欧周遊（2026-09-12 〜 09-27）。`ORDER` の 19〜24 と1対1。
    # **オーランド（Åland）は入れない。** フィンランドの自治領だが、
    # ヘルシンキ→ストックホルムの夜行船が脇を通っただけで降りていない。
    # 塗ると「歩いた」ことになる。
    "Poland": "poland",
    "Lithuania": "lithuania",
    "Latvia": "latvia",
...
# ---------------------------------------------------------------- 投影
def unseam(pts: list) -> list:
    """日付変更線の継ぎ目を外した輪を返す。

    世界の陸（`land`）は、**アフリカ＋ユーラシアでひと続きの輪**になっていて、
    チュクチ半島が日付変更線に届く。元データはそこを

        … (179.77, 68.99) → **(-180, 68.98) → (-180, 65.07)** → (179.90, 65.05) …

    という「幅ゼロの橋」で閉じている。地球儀の上では同じ場所だが、
    **経度の数の上では地球を半周する線**。切り出し（`clip`）は経度をそのまま
    引き算するので、この2本が**窓を端から端まで横切る線**になり、
    その北側がまるごと陸として塗られる。

    枠の北が 60.5 度だったころは、継ぎ目の緯度（65.07 と 68.98）が窓の外
    （pad 込みで 64.6 度まで）だったので、切り出しが丸ごと捨てていた。
    **2026-09-22 に 70.5 度まで広げたら、ノルウェー海の上に緯度 65〜69 の
    偽の陸が現れた**（左端から北欧の海岸まで、まっすぐな南の縁を持つ帯）。

    直しかたは、**経度が 180 度以上飛ぶところで輪を切り、いちばん長い
    かけらだけを閉じ直す。** 捨てられるのは継ぎ目の2点だけで、残った端どうしは
    どちらも経度 179.8 付近なので、チュクチは日付変更線に沿って素直に閉じる。

    この地図の窓は経度 -11〜57.5 で、**日付変更線を含まない。**
    含む窓を描くようになったら、この関数では足りない（かけらを両方とも
    描き分ける必要がある）。
    ...
    Returns:
        継ぎ目の無い輪。飛びが無ければそのまま
    """
    cut = [i + 1 for i in range(len(pts) - 1) if abs(pts[i + 1][0] - pts[i][0]) > 180.0]
    if not cut:
        return pts
    # 切れ目で分けて、いちばん点の多いかけらを採る
    pieces = []
    prev = 0
    for c in cut:
        pieces.append(pts[prev:c])
        prev = c
    pieces.append(pts[prev:])
    # 輪なので、最後のかけらと最初のかけらはつながっている
    if len(pieces) > 1:
...
def girdles(pts: list) -> bool:
    """地球をぐるりと一周する輪か。**描けるものではないので捨てる。**

    Natural Earth の陸（`land`）には、**北緯 71 度あたりを経度 -180 から 180 まで
    ひと回りする、高さ 0.7 度の細い輪**が1つ混じっている（55点）。
    そんな陸は無い。継ぎ目の産物。

    枠の北が 60.5 度だったころは、切り出す窓（pad 込みで 64.6 度）より上に
    あったので `clip` が3点未満にして勝手に消えていた。**2026-09-22 に北欧を
    入れるため 70.5 度まで広げたら、初めて窓に入ってきた**——窓いっぱいの幅で
    高さ 25px の紙のような輪になり、`rdp` が4点に丸め、海岸線の帯
    （太さ 34 のぼかし＋16＋9＋6.5＋3.5 の5枚重ね）が地図の上半分に
    **大きな弧**として滲み出した。

    捨てる条件は**「経度をほぼ一周していて、しかも緯度に厚みが無い」**こと。
    **一周しているだけでは捨てない。** アフリカ＋ユーラシアの本体もひと続きの
    輪で、ロシアが日付変更線をまたぐので経度は -180〜180 ある。そこだけで
    切ると**陸そのものが消える**（試したら land が 58KB → 38KB、森が
    1,379 → 43 になった）。本体は緯度が 100 度以上あるので、厚みで分かれる。

    切り出したあとの形ではなく**元の輪**で見る。切ったあとでは、ただの
    横長の帯と区別がつかない。

    Args:
        pts: 切り出す前の (経度, 緯度) の並び
    Returns:
        一周する紙きれなら True
    """
    if len(pts) < 3:
    ...
    return max(lons) - min(lons) >= 355.0 and max(lats) - min(lats) <= 5.0
...
CITIES = [
    ...
    # 北欧周遊。**泊まった街だけ**（`content/countries.ts` の `stays` と同じ並び）。
    # カトヴィツェは夜中に着いた空港なので `stop`、ピンはワルシャワに立てる。
    ("katowice", "カトヴィツェ", 19.0238, 50.2649, "poland", "stop"),
    ("warsaw", "ワルシャワ", 21.0122, 52.2297, "poland", "hub"),
    ("bialystok", "ビャウィストク", 23.1450, 53.1325, "poland", "stop"),
    ("vilnius", "ヴィリニュス", 25.2797, 54.6872, "lithuania", "hub"),
    ("riga", "リガ", 24.1052, 56.9496, "latvia", "hub"),
    ("tallinn", "タリン", 24.7536, 59.4370, "estonia", "hub"),
    ("helsinki", "ヘルシンキ", 24.9384, 60.1699, "finland", "hub"),
    ("stockholm", "ストックホルム", 18.0686, 59.3293, "sweden", "hub"),
]
...
LEGS = [
    ...
    # 北欧周遊。クタイシから飛んで、**あとはぜんぶ人の車と船**。
    # バルト海の2本を `sea` にしてあるのは、タリン→ヘルシンキもヘルシンキ→
    # ストックホルムも船だから（`content/nordic.ts` の `ferry`）。
    ("kutaisi", "katowice", "air", 0.10),
    ("katowice", "warsaw", "hitch", 0.12),
    ("warsaw", "bialystok", "hitch", -0.10),
    ("bialystok", "vilnius", "hitch", 0.11),
    ("vilnius", "riga", "hitch", -0.12),
    ("riga", "tallinn", "hitch", 0.10),
    ("tallinn", "helsinki", "sea", 0.16),
    ("helsinki", "stockholm", "sea", -0.12),
]
...
# 章。地図を寄せるときの範囲（経度緯度）。
#
# 名前は「◯◯編」ではなく地方の名前だけにする。ボタンは4つ並ぶので、
# 「編」を付けると 390px の画面で2行に折り返す。しかも画面の言葉は
# 「上のボタンで地図を寄せられます」なので、章立てではなく寄せ先の名前。
# 中東はエジプトを含む（前は「中東・アフリカ編」だった）。地図を見れば
# どこまで入っているかは見えるので、名前は短いほうを取る。
CHAPTERS = [
    ("all", "ぜんぶ", LON_MIN, LON_MAX, LAT_MIN, LAT_MAX),
    # **北欧の6カ国はここに入る。** `content/countries.ts` の `region` が
    # 6つとも「ヨーロッパ」で、`/map` の年表はこの id で段を作っている
    # （`app/map/page.tsx` の `CHAPTER_OF`）。**ボタンと段は同じ表から出るので、
    # 段に居る国がボタンの枠の外にいると、押した先にその国が写らない。**
    # 前は東 22.5 / 北 58.6 で、ヴィリニュス(25.3E) リガ(24.1E) タリン(24.8E)
    # ヘルシンキ(24.9E, 60.2N) が**4つとも枠の外**だった。
    ("europe", "ヨーロッパ", -8.0, 27.0, 41.0, 61.5),
    ("mideast", "中東", 23.0, 56.0, 21.0, 43.0),
    ("caucasus", "コーカサス", 38.0, 51.5, 37.6, 44.4),
]
...
ZOOM = {
    ...
    # 北欧の6カ国。**世界1枚の枠（`LAT_MAX`）とは関係が無い。** 寄りの地図は
    # 世界地図を拡大したものではなく、この範囲で投影し直したものなので、
    # ここは「旅の中身が見える範囲」だけで決める（表の他の国と同じ）。
    "poland": (13.9, 24.4, 48.8, 55.0),
    "lithuania": (20.6, 27.2, 53.6, 56.7),
    "latvia": (20.6, 28.5, 55.5, 58.4),
    "estonia": (21.4, 28.5, 57.3, 60.0),
    # **フィンランドとスウェーデンは、国の輪ぜんぶを入れない。** 北緯70度まで
    # 伸びているので、そのまま取ると縦長すぎて `fit_aspect`（横 1.5 倍）が
    # 左右に 40 度ぶん海と隣国を足す。国が真ん中の細い帯になって、
    # 寄ったのに何を見ているのか分からない。旅が触ったのは南の端なので、
    # **首都のまわりと、国の形が読めるところまで**にする。
    "finland": (19.4, 31.6, 59.4, 65.2),
    "sweden": (10.4, 24.6, 55.0, 62.4),
...
def build(
    ...
    def rings_of(geom: dict) -> list:
        ...
        for poly in polys_of(geom):
            for ring in poly:
                raw = unseam(ring_points(arcs, ring))
                if girdles(raw):
                    continue
                pts = clip(raw, box)
                if len(pts) < 3:
