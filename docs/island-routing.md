...
面の一覧は**手で並べない**（§8）。本番の `sitemap.xml` と、リポジトリの
`site/app/**/page.tsx` の静的な道（`[` を含む道は除く）から出して足す。
**サイトマップに無い7面に `/roulette` が入っている**——配信に映る面なので落とせない。

対照は8本で、足を1本ずつ抜くのは `BREAK=status|h1|title|thin|follow|tell`。
判定だけを切り出した見張りが `tools/sprites/prodsweep_selftest.mjs`（毎 PR で走る）。
**Hosting を配ったあとに `firebase-hosting-deploy-prod.yml` から自動で走る。**

実測（2026-09-19。本番）:

```
面の一覧: sitemap 119面 ＋ サイトマップに無い静的な道 7面 = 126面
  HTTP が 200          126面中 126面 / 違反 0
  h1 がちょうど1つ     126面中 126面 / 違反 0
  <title> が空でない   126面中 126面 / 違反 0
  本文が 4096B 以上    126面中 126面 / 違反 0
  追ったことが表に出る 126面中 126面 / 違反 0
  リダイレクト         追った 1面（/nordic/photos → /cards。設計どおり）
読めなかった 0面 / 6秒
```
