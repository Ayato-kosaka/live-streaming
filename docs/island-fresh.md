...
### 毎晩に入れていない理由と、それでも古くなって困らない理由

700本あまりを1本ずつ当てると **25〜30分**かかる。毎晩の焼き直し（7本）にそれを足すと、
**焼き直しそのものが YouTube の機嫌で落ちる。** 焼き込みは BigQuery に入っている
ところまでを焼くのが仕事で、外の相手が黙った晩に止まってよいものではない。
...
**取り置きを新しくするのは人の仕事。** 見張り（`python/dead_stream_watch.py`）が
**1（見られないものがあった）を返したとき**に回す。

```bash
python3 python/build_dead_streams.py      # 25〜30分。対照を先に回してから測る
```
