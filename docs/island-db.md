...
| 豚の貯金箱の元（鍵・起点・スパチャ・目標） | GAS の `Goals` 表 | Firestore `islandGoal/2025-10-24` | **GAS が正**（#305）。スパチャを書き足す OBS の書き先がそこしかなく、**額が伸びるのは表の側だけ**だから。サイトは GAS → 無ければ Firestore の順に読む（**ただし GAS がこけた回は控えに落ちない。** 前に GAS から取った値を返し続ける。#604。下の b を見る）。控えは `goal_migrate` で作る。**表を消した瞬間から控えが正になる。** 順を入れ替えてよくなるのは、`SuperChats` の書き込み先を移したあと |
...
**読めなかったときに 0 を返さない。** そして **GAS がこけた回に、ここへ
落ちない**（#604）。手元に GAS から取った値があるなら、それを返し続ける。

GAS への読みは一時的にこける。落ちるたびにここ（9日前のことがある）を
掴むと、**同じ時刻に叩いても額が行ったり来たりする。** 実際に 63,330円 と
80,970円 が 17,640円 差で行き来していた。落ちてよいのは**キャッシュが
1つも無いとき**（冷たいインスタンスの初回）だけ。
...
点を今日の顔ぶれに化かすのは `roster.ts`。あやとの決め（2026-09-19 / #568）で、
**席を2つに分けてある。**

| | |
| --- | --- |
| **点0の人**（直近90日で出席0日・投げ銭0） | **島に出さない。** 本番の103人のうち59人がこれ |
| **固定席**（`FIXED_SEATS`。いま 4席） | 点の上から順に、**毎日そのまま** |
| **抽選席**（残り。11人の島なら7席） | 点を重みにした日替わりの抽選（`TOP_WEIGHT`。いま 3倍） |

**倍率（`TOP_WEIGHT`）だけでは「上位は固定気味」は作れない。** 上位と下位に
同時に効くので、上げると下も動かなくなる。実測でも、固定席を0にして倍率を
3倍→30倍にしても点0.9超の人が島にいる率は 30% → 33% しか動かなかった。
測りかたは `site/selftest/roster_selftest.mjs`。

**名簿が痩せた日**（新しい章の頭・取り込みが落ちた日）で点>0 が定員に届かない
ときだけ、点0の人から日替わりで穴を埋める。**空の島は出さない。**
