...
- **住人の「一緒にいた日数」を自動にする。** いまは `residents.ts` に手で書いている。
  キャラクター画像とチャンネルの対応（いまはスプレッドシート）を Firestore に移せば、
  `islandUsers.channelId` と BigQuery の `author_channel_id` がつながって自動になる。
- **企画ページを Firestore から出す。** いまはコードに置いて Claude Code で磨く形。
  数が増えて磨く余裕がなくなったら、下書きをそのまま公開できるようにする。
- **国ごとの企画募集を独立させる。** いま `islandIdeas` に札を付けて相乗りしている。
  企画が増えて掲示板が読みにくくなったら、`countryId` の列を足して分ける。
