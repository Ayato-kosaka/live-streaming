...
そのときの記録。**戻すというのは、取り出して次の4つに通ることまで。**

| 見るもの | なぜ |
| --- | --- |
| 大きさが記録と合う | 途中で切れた実体 |
| 指紋（sha256）が記録と合う | 置き場の中で腐った実体 |
| **0 バイトでない** | 空を積むと `size` も `sha256` も**空のそれ**になる。辻褄は合う |
| **頭が絵になっている** | 合言葉の切れた URL から返った HTML も、辻褄は合う |

**下の2つが要るのは、`size` と `sha256` を実体から計算して一緒に書いている
から。** 取り違えた中身は**行の中で辻褄が合ったまま**入り、突き合わせは
一致と出て、戻した先に絵は無い。**それを退避と呼ばない。**

```bash
# 何枚か戻して、バイト列が一致するか見る（**置き場を読むだけ**）
python python/backup/restore.py --photo

# 枚数を変える（既定5枚。`PHOTO_DRILL_N` でも変えられる）
python python/backup/restore.py --photo --photo-n 20

# どの1枚かを指定して、実体をファイルに書き出す
python python/backup/restore.py --photo \
  --photo-path "nordic/photos/2026-09-12/xxxx.jpg" --photo-out /tmp/one.jpg
```

終了コード 0=戻せた / 1=戻せない写真があった / **2=数えるものが無い**
（置き場が0枚・名指しした1枚が無い）。

**毎晩これが `backup.yml` の `drill` で自動で回っている。**
引くのは**晩ごとに違う組み合わせ**（`path` を日付で並べ替えて上から n 枚）。
いちばん新しい1枚だけを見ていると、584 枚のうち同じ側を毎晩なぞることに
なって、**古いところが腐っても永久に当たらない。**

**0枚の晩は赤くする。** 2026-09-17 まで、ここは1枚も無い晩を `::warning::` で
緑のまま通していた。「戻せた」ではなく「**何も戻していない**」で、
いちばん見張ってほしい晩に黙る形だった（`docs/island-misses.md` #127）。
...
4. **取るだけでなく、毎晩「戻せるか」も試している。**
   ...
   件数と中身の指紋を突き合わせ、**写真も何枚か戻してバイト列を突き合わせる。**
   **壊れた退避で緑を出さないため**
5. **その演習が落ちられることも、毎晩その場で見せている。**
   `python/backup_drill_selftest.py` が、偽の置き場に**壊した写し**
   （1バイト違う・欠けた・空っぽ・絵ではない・0枚・戻したのに中身が違う）を
   仕込んで、そこで落ちることと、**壊していない写しが先に通ること**を
   両側から当てる。落ちたら**本番の数字を1つも出さずに**赤くなる。
   19 run 捕まえていないのが「健全」なのか「見ていない」なのかは、
   走った回数からは決められない（`docs/island-misses.md` #127）
