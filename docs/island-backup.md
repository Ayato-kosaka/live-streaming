...
| **旅の写真の実体**（`nordic/photos/…`・544枚 340MB） | **人が撮ったもので、どこからも作り直せない。** 参照だけ取って実体を取らないのは、鍵だけ持って家を持たないのと同じ |
...
- **Firestore の2つの入れ物に載っていない実体には、1枚も届かない。**
  置き場ぜんぶで **544件・340,231,177 バイト**（Functions のサービス
  アカウントで `objects.list` を叩いた実測・2026-09-12）。
  **そのうち何件が索引に載っているかは、まだ測っていない。**
  退避のログが出す「実体 N 枚」と 544 の差が、この道では届かないぶん。
  差が出たらここに実測を書く（**測っていないことは測っていないと書く**）
...
**この日の 164MB に、写真は1バイトも入っていない。** 実体（544枚 340MB）が
入りはじめるのは 2026-09-12 に `photos.py` を入れてから。**入り切れば
置き場は 約 500MB になる**（東京の置き場代で月 1〜2円）。
