...
/** 表彰台。月末配信の「授賞式」の絵（#149）。
 *
 * **前は噴水だった。** 168px で出ていたころは何の絵か分からなかったが、
 * 図鑑の主役として 561px まで大きくしたら、**中身の無い大きな水盤**にしか
 * 見えなくなった。同じ面の流れの印はメダルとトロフィーなのに、主役だけが
 * 噴水で、面の中で言っていることが2つに割れていた
 * （`docs/island-design.md` 6章「絵がその名前を指しているかを見る」）。
 *
 * **Kenney のキットにトロフィーは無い**ので、箱で組む。台は3段にした。
 * トロフィー1つだけだと「1位」の絵になってしまうが、この配信で表彰するのは
 * 出席・名言・おもしろコメント・投げ銭の4つで、**1人を選ぶ回ではない。**
 * 段が3つあると「並べて讃える場」に見える。
 *
 * 段の高さは 0.52 / 0.36 / 0.24。差を付けすぎると 96px の丸に入れたときに
 * いちばん低い段が潰れるので、いちばん低いものでも台の厚みが分かる高さで止める。 */
const awardStage = () => [
  // 台。中央がいちばん高い。手前の面だけ明るくして、奥行きを出す
  box([0.46, 0.52, 0.42], C.white, [0, 0.26, 0]),
  box([0.42, 0.36, 0.42], C.white, [-0.45, 0.18, 0]),
  box([0.42, 0.24, 0.42], C.white, [0.45, 0.12, 0]),
  // 段の縁。板の色を1本入れると、白い箱3つが1つの塊に見えなくなる
  box([0.47, 0.05, 0.43], C.plank, [0, 0.525, 0]),
  box([0.43, 0.05, 0.43], C.plank, [-0.45, 0.365, 0]),
  box([0.43, 0.05, 0.43], C.plank, [0.45, 0.245, 0]),
  /* トロフィー。台座 → 柱 → 杯 → 取っ手2つ。
     **杯は1つの箱にしない。** 四角い金の塊にしか見えず、撮って気づいた。
     幅の違う板を3枚積んで、上へ広がる形（すぼまった杯）を作る。 */
  box([0.17, 0.06, 0.17], C.gold, [0, 0.58, 0]),
  box([0.06, 0.10, 0.06], C.gold, [0, 0.66, 0]),
  box([0.13, 0.05, 0.12], C.gold, [0, 0.735, 0]),
  box([0.17, 0.05, 0.15], C.gold, [0, 0.785, 0]),
  box([0.21, 0.06, 0.18], C.gold, [0, 0.840, 0]),
  // 取っ手。杯の外へ出す。細くしないと、杯と1つの塊に見える
  box([0.03, 0.12, 0.03], C.gold, [-0.125, 0.795, 0]),
  box([0.03, 0.12, 0.03], C.gold, [0.125, 0.795, 0]),
  // 低い2段には賞の印。金と同じ色にすると台の上が全部トロフィーに見える
  box([0.13, 0.13, 0.03], C.red, [-0.45, 0.46, 0.19]),
  box([0.13, 0.13, 0.03], C.blue, [0.45, 0.34, 0.19]),
];
...
const SPRITES_BASE = [
  ...
  /* 月末配信の授賞式（#149）。島には置かない。図鑑の主役としてだけ出る */
  { name: "stage-award", parts: awardStage(), opts: { plain: true } },
...
/* 主役は 1280px の画角で焼いて、meta.py が 640px へ縮める。
 *
 * **640 で焼いてはいけない。** render.html は SUPER=2 で焼くので画角は
 * 1280px になるが、物がその中いっぱいに写るとは限らない。噴水は 538px しか
 * 写っておらず、meta.py の「長辺 640px まで」に最初から収まってしまって、
 * 1画素も縮まずに webp になっていた。縮めないということは平らにしない
 * (アンチエイリアスが効かない)ということで、焼いたときの網目——接地影の
 * グラデーションに出る点々——がそのまま残る。PC の dpr2 ではその絵を
 * 2倍に伸ばして出すので、紙の上の汚れに見えていた。
 *
 * 1280 にすると画角は 2560px になり、噴水は 1078x653 で写る。
 * 640px へ 1.7倍縮むので網目は均され、絵も 538x327 → 640x388 に増える。
 * 焼くのに時間はかかるが、図鑑の主役は面に1枚しか出ない。 */
const HERO_PX = 1280;
...
/** 図鑑に出る絵の名前。主役（hero）と同じ顔ぶれ。 */
const LIST_NAMES = new Set(heroNames());

/* 図鑑に出る絵は、**基本のほうも上限まで焼く。**
 *
 * `/kitchen` の一覧のマスは、絵を幅 189px まで出す。高精細画面では 378px
 * 要るのに、焼けていたのは 202px（`food-bacon`）だった。1.87倍に
 * 引き伸ばしていたことになる（#150。`tools/sprites/stretch.mjs` で測った）。
 *
 * **上限（meta.py の MAX_SIDE = 320）に当たっていたのではない。** 画角の中で
 * 物が小さく写るので、切り出した時点で 202px しか無かった。主役（hero）で
 * 踏んだのと同じ話で、`px` を上げて「上限まで縮む」状態にすれば直る。
 *
 * 640 にすると画角は 1280px。いちばん小さく写る料理でも 400px を超えるので、
 * meta.py が必ず 320px へ縮める。縮むということは平らになる（アンチエイリアス
 * が効く）ということでもある。 */
const LIST_PX = 640;

export const SPRITES = [
  ...SPRITES_BASE.map((s) => (
    LIST_NAMES.has(s.name) ?
      {...s, opts: {...(s.opts ?? {}), px: LIST_PX}} :
      s
  )),
  ...heroNames().map((name) => {
