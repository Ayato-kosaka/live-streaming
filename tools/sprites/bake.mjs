...
import { SPRITES, COLORMAP } from "./manifest.mjs";
...
for (const s of list) {
  ...
  const opts = { colormap: s.colormap ?? COLORMAP, outline: s.outline ?? 0.011, ...(s.opts ?? {}) };
  const data = await p.evaluate(([pp, oo]) => window.renderScene(pp, oo), [parts, opts]);
