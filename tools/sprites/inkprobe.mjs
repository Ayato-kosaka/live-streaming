/**
 * `inkpx.mjs` の**自己確認つき**。撮り方はそのままで、測る前に
 * **濃さの分かっている字を2本差し込む**（`docs/island-misses.md` #19、
 * 「数え方が届いていない場所は、0件ではなく『数えていない』」）。
 *
 *   PORT=4140 TAG=probe PAGES=/about node tools/sprites/inkprobe.mjs
 *   python3 tools/sprites/inkpx.py probe _about 4.5
 *
 * 差し込む2本は白地（#ffffff）の上の灰色で、計算上の比が
 *   #7f7f7f … 4.00（**4.5 を割るので、挙がらなければ道具が届いていない**）
 *   #6f6f6f … 5.02（**4.5 を超えるので、挙がったら道具が厳しすぎる**）
 * 既存の `inkpx.mjs` は書き換えていない（前の担当の測定が再現できなくなるため）。
 *
 * 島は rAF で動く。2枚のあいだに1画素でも動くと差分に字以外が混ざるので、
 * **撮る前に rAF を止める**（`CLAUDE.md`「島を止めてから撮る」）。
 *
 * 以下、撮り方は `inkpx.mjs` と同じ。
 *
...
for (const path of PATHS) {
  ...
  /* **島を止めてから撮る。** CSS の animation を止めるだけでは足りない。
     島の住人とカメラは rAF で動くので、2枚のあいだに歩かれると
     「地が紫（住人の服）」のような値が出る（`CLAUDE.md`）。 */
  await p.evaluate(() => {
  ...
  /* 濃さの分かっている字を2本置く。**足元の砂浜のすぐ上**に置くので、
     測る対象と同じ1枚の中に写る。 */
  if (process.env.PROBE) {
    await p.evaluate(() => {
      const host = document.querySelector(".ifoot") || document.body;
      for (const [id, col] of [["inkprobe-400", "#7f7f7f"], ["inkprobe-502", "#6f6f6f"]]) {
        const d = document.createElement("p");
        d.id = id;
        d.className = id;
        d.style.cssText =
          `background:#ffffff;color:${col};font-size:20px;font-weight:900;` +
          `margin:0;padding:8px;text-shadow:none`;
        d.textContent = "ためしの字 ためしの字";
        host.appendChild(d);
      }
    });
