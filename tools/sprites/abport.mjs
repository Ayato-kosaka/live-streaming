/**
 * 直す前と後を、**2つのポートで交互に**測る。
 *
 *   A=4502 B=4503 node abport.mjs
 *
...
for (let i = 0; i < N; i++) {
  for (let j = 0; j < PORTS.length; j++) out[j].load.push(await load(PORTS[j][1]));
  for (let j = 0; j < PORTS.length; j++) out[j].idle.push(await idle(PORTS[j][1]));
}
await b.close();
...
for (let j = 0; j < PORTS.length; j++) {
  ...
  console.log(`  ${PORTS[j][0]}(:${PORTS[j][1]})  FCP ${med(L, "fcp").toFixed(0)}ms  LCP ${med(L, "lcp").toFixed(0)}ms  起動CPU ${med(L, "cpu").toFixed(0)}ms(主 ${med(L, "main").toFixed(0)})  転送 ${med(L, "bytes").toFixed(0)}KB/${med(L, "reqs")}本`);
