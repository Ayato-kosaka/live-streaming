/**
 * 島の面の字の濃さを測る（撮るほう）。`inkpx.mjs` と同じ2枚撮りだが、
 * **島を止めてから撮る。**
 *
 *   SPORT=4310 TAG=isle node tools/sprites/isleink.mjs
 *   python3 tools/sprites/inkpx.py isle _island_nordic
 *
 * 島は rAF で動く（あやとが歩き、カメラが寄る）。CSS の animation を
 * 止めるだけでは足りないので `requestAnimationFrame` を潰す（`CLAUDE.md`）。
 * 止めずに撮ると、2枚のあいだに住人が動いたぶんが「字の画素」に混ざる。
 * 読むほうは `inkpx.py` をそのまま使う（出す形を合わせてある）。
 */
import { chromium } from "playwright-core";
import { offline } from "./route.mjs";
import { mkdirSync, writeFileSync } from "fs";

const PORT = process.env.SPORT || process.env.PORT || "4310";
const PATHS = (process.env.PAGES || "/island/nordic.html,/island/caucasus.html,/island/albania.html,/island/europe.html").split(",");
const TAG = process.env.TAG || "isle";
...
for (const path of PATHS) {
  ...
  // **島を止める。** 2枚のあいだに住人が動くと、その画素が字として数えられる
