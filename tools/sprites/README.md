...
| `og.mjs` | 分かち合うときに出る絵（`site/public/og.png`、1200×630）を撮り直す。**顔を先に落としてから**（`avatars.py` / `chars.py`）。終わりに `ogcheck.py` を呼ぶ |
| `ogcheck.py` | その絵が**ちゃんと島に見えるか**を画素で（海・陸・ひろがり・のっぺり・色の種類）。対照11枚。0/1/2 |
| `talk.mjs` | 住人に話しかける流れ |
| `nmap.mjs` | 北欧のルート地図だけ |
...
## 図鑑の「もらったカード」から、その人のカードぜんぶへ行けるか

```bash
node tools/sprites/cardgo.mjs                  # 本番の面を見る（既定。**ビルド要らず**）
PORT=4170 node tools/sprites/cardgo.mjs        # 手元の書き出しを見る（先に tools/build.sh 3170）
```

0＝通った / 1＝行けない人がいた・札の字が行き先と食い違っていた /
2＝数えるものが無い（本番の口が引けない・図鑑が出ない・外に出られなかった先がある）。

**枚数のもとは、どちらのモードでも本番の口**（`GET /cards`）から取る。差し込み
（`asme.mjs`）のカードは1人3枚までなので、**43枚の人が居ない**まま「溜まったときに
壊れるもの」を測ることになる（`docs/island-misses.md` #177）。

対照（当てると落ちる。**足は1本ずつ折る**）:

| | 何を壊す | 落ちかた（本番の実測。2026-09-19） |
| --- | --- | --- |
| `BREAK=cut2` | 3枚目から先と押しどころを隠す（直す前の姿） | 1 / 行けない人 17件だけ |
| `BREAK=golink` | 欄に「この人のカードを、ぜんぶ見る」→ `/cards` を置く | 1 / 札の字 22件だけ |
| `STARVE='_next/static'` | 本番モードで、束ねた JS を止める | 2 / 外に出られなかった先（数字を1つも出さない） |
| `CHROME=/nowhere/chrome` | ブラウザの在りかを外す | 2 / 「ブラウザが無い（見た先: …）」 |

判定だけは `cardgo_selftest.mjs` が毎 PR で当てている（ブラウザも本番も要らない）。

**版を名指ししない。** ブラウザの在りかは `tools/sprites/chrome.mjs` が探す
（`CHROME=` で1本を名指しもできる）。置き場に在る `chromium-<番号>` のうち
**番号のいちばん大きいもの**を採るので、`playwright-core` を上げても外れない。
無ければ 2（測れていない）で止まる——**1 にしない。**
道具が配りの後ろ（`firebase-hosting-deploy-prod.yml`）で走るようになって、
あそこは**その場でブラウザを落としている**ので、番号を書くと上げた日に静かに死ぬ。
**他の 213本は、まだ直に書いてある**（2026-09-19 の実測。うち 206本は
`executablePath:` に直に置いている）。`CLAUDE.md` のとおり版は 1.56.0 で止めて
あるので今は合っているが、**CI で無人で走るのはここだけ**なので先にこちらを外した。
